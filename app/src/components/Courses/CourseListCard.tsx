...
export const CourseListCard = ({ course }: { course: Course }) => {
  const [coverImage, setCoverImage] = useState(imageNotFoundImage);
  ...
  useEffect(() => {
    const getImageBinary = async () => {
      let image: string;
      try {
        if (course.coverImg !== undefined && course.coverImg !== "") {
          const imgBinary = await StorageServices.getFile(
            course.coverImg,
            userToken
          );
          image = `data:image/jpeg;base64,${imgBinary}`;
        } else {
                ...
                <div className="flex flex-row">
                  <div
                    className={
                      ...
                      (statuses[course.status].color ?? statuses.default.color)
                    }
                  />
                  <p className="italic">
